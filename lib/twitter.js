...
class Twitter extends State {
  ...
  async _post({ id, user }) {
    ...
    //console.log('User\'s Images Object: ', user.images);
  ...
  async addImage(imageUrl, messageId, user) {
    ...
    const media = await this.client.post('media/upload', { media: image.data }).catch(err => console.log('Error from line 42: ', err));
